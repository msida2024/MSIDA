...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        # processed memory and coord proposals
        bs, _, c = memory.shape
        output_memory, output_proposals = self.gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes)

        # bezier coord offsets and class proposals, use MLP and Linear
        enc_outputs_class = self.bezier_class_embed(output_memory)  # predict class
        enc_outputs_coord_unact = self.bezier_coord_embed(output_memory) + output_proposals  # predict coord

        # combine class and semantic
        enc_outputs_text = self.text_class_embed(output_memory)
        enc_outputs_text = enc_outputs_text[:, :, -1].unsqueeze(-1)
        enc_outputs_class = enc_outputs_class * enc_outputs_text

        # select top-k curves
        topk = self.num_proposals  # num of K
        topk_proposals = torch.topk(enc_outputs_class[..., 0], topk, dim=1)[1]  # choose 100 coord depending on class
        topk_coords_unact = torch.gather(enc_outputs_coord_unact,
                                         1, topk_proposals.unsqueeze(-1).repeat(1, 1, 8))  # extra coord offset
        topk_coords_unact = topk_coords_unact.detach()
        reference_points = topk_coords_unact.sigmoid()  # bs, nq, 8: (bs, 100, 8)
        reference_points = self.init_points_from_bezier_proposals(reference_points)  # bs, nq, num_points, 2: (bs, 100, 25, 2)
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        q = k = self.with_pos_embed(tgt, query_pos)  # Cq + Pq
...
class DeformableCompositeTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            query_pos = gen_point_pos_embed(reference_points_input[:, :, :, 0, :], self.d_model, self.temp)  # PE
            query_pos = self.ref_point_head(query_pos)  # 2-layer MLP
